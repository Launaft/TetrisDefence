...
{
    internal class CommonBlock : Block
    {
        public override event EventHandler IsDestroyed;

        private int _x, _y, _size = 40;
        private double _hitPoints = 1.0;
        private ImageBrush _image = new ImageBrush(new BitmapImage(new Uri("pack://application:,,,/Resources/block.png")));
        private Rectangle _rectangle = new Rectangle();

        public CommonBlock(int x, int y)
        {
            _x = x;
            _y = y;
